...
## 0.42.2-dev-fork.1 (2026-06-08)

Fork sync: integrate upstream/develop @ 047f454 (56 commits). Highlights:

* **mvn:** new Rust filter module replacing the TOML filter, adds test-phase support, Surefire 3.x failure-trail handling, multi-module Reactor summaries, and `-q` quiet-mode filtering (upstream #1956).
* **permissions:** CVE/permission hardening port — never auto-allow non-evaluable commands (defer to host), project-first config lookup + correct settings files for Gemini/Cursor, `>&file` redirect no longer auto-allowed.
* **curl:** passthrough binary downloads to prevent UTF-8 corruption.
* **aws:** preserve values in JSON output for unsupported subcommands.
* **cmds:** strip decorator noise from filter output across modules.
* **helm:** remove `max_lines` cap that truncated template output.

Fork version base bumped from `0.41.0` → `0.42.2` (upstream `0.42.2`), fork counter reset to `.1`.

---

## 0.41.0-dev-fork.0 (2026-05-27)

Fork sync: integrate upstream/develop @ 5a149a7 (58 commits). Highlights:

* **grep:** fix parser for content containing `:` / `::` (upstream #1436). Eliminates phantom `[file] N` buckets on patterns like `ClassRegistry::init`. NUL-separated rg output (`-nH --null`) + new regex-based `parse_match_line`.
* **git status:** drop `-uall` from compact path so output never exceeds raw (upstream `7753e48`). Fixes ~29× blowup on directories like `node_modules/`.
* **git log:** honor explicit `-n N` on merge HEAD (upstream #2009). No longer silently returns the second parent's SHA.
* **truncate:** new `src/core/truncate.rs` centralizing `CAP_LIST` / `CAP_ERRORS` / `CAP_WARNINGS` across ~30 cmd modules.
* Upstream Pi-agent integration lands inert (gated on `--agent pi`).
* License: align with upstream Apache-2.0.
