...
| **Network egress — HTTP client** | `ureq` (the sole HTTP/TLS crate) is declared `optional = true` in `Cargo.toml` and is only activated by the `telemetry` feature; the enterprise build does not activate `telemetry`, so `ureq` is never compiled in. `cargo tree --features enterprise` confirms no HTTP/TLS crate reaches the resolved dependency graph. `build.rs` adds a compile-time guard that fails the build if someone activates both `enterprise` and `telemetry` simultaneously (the one configuration that would smuggle networking crates back in). `deny.toml` adds a second, independent supply-chain enforcement layer via `cargo deny check bans`, effective on the enterprise distribution where `ureq` has been removed from `Cargo.toml` entirely. | **Primary proof:** full command surface run under a deny-all network namespace (`unshare -rn` on Linux, `sandbox-exec` deny-network on macOS) — zero connection attempts observed. Captured as a re-runnable CI artifact. Secondary: `nm -C target/release/rtk \| grep -i ureq` → empty; `strings target/release/rtk \| grep -iE 'ureq\|telemetry\|rtk-ai\.app'` → empty; `cargo tree --features enterprise \| grep -iE 'ureq\|reqwest\|tokio\|hyper\|tls\|openssl\|sentry'` → empty. |
