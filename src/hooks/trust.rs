...
///
/// Hashing the exact bytes that are returned closes the TOCTOU window: callers
/// must execute the returned content rather than re-reading the file (which an
/// attacker could swap between the trust check and the read).
