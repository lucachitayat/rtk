...
/// Like `cursor_allow` but asks the Cursor host for human approval instead of
/// pre-approving. Serves upstream's `HookDecision::AskRewrite` and, in this fork,
/// every `AllowRewrite` for which `auto_allow_enabled()` returns `false`.
///
/// This tracks upstream's explicit `"permission": "ask"` rather than the fork's
/// earlier omit-the-field form. Both prompt the human — omitting made the host
/// fall back to its own prompt — so approval is unchanged, and matching upstream
/// removes a divergence that conflicted on every sync.
...
mod tests {
    ...
    // FORK-ONLY: hardening / enterprise tests are quarantined in their own file so this
    // upstream-owned module stays mergeable — see src/hooks/hook_cmd/tests/fork_tests.rs
    // for the rationale. Declared *inside* `mod tests` (not at file scope) so its
    // `use super::*` still reaches this module's private helpers (cli_args, claude_input,
    // cursor_input, all_allowed, gemini_render) as well as hook_cmd's private items.
    mod fork_tests;
