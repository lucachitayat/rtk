...
/// One entry observed by a walk worker. Workers do no bookkeeping of their own; they
/// hand back the raw facts and `native_walk` replays them serially.
struct WalkRecord {
...
fn native_walk(
    ...
    // Parallel walk (the same engine fd/ripgrep use) — fork-local perf work: on a 16k-file
    // tree this took `rtk find` from 270ms to 135ms. Each worker owns a cloned sender and
    // cloned read-only inputs, so the visitor is `Send`. Workers only *observe*; the
    // disclosure bookkeeping below is replayed serially over the drained records, so the
    // result is identical to a serial walk regardless of thread scheduling.
    ...
    // All senders are dropped once `run` returns (it joins the workers and drops the
    // factory), so this iterator terminates.
    let records: Vec<WalkRecord> = rx.into_iter().collect();
    let disclose = seen.load(Ordering::Relaxed) <= DISCLOSURE_ENTRY_CAP;

    let mut files = Vec::new();
    let mut visited: HashSet<std::path::PathBuf> = HashSet::new();
    let mut visited_dirs: Vec<(std::path::PathBuf, usize)> = Vec::new();
    for rec in records {
        if let Some(display) = rec.display {
            files.push(display);
        }
        if disclose {
            if rec.is_dir {
                visited_dirs.push((rec.path.clone(), rec.depth));
            }
            visited.insert(rec.path);
        }
    }
    // Workers finish in arbitrary order; sort so `disclose_filtered`'s FILTERED_CAP
    // early-return truncates a deterministic prefix. (`files` is sorted by `render`.)
    visited_dirs.sort();
...
mod tests {
    ...
    // --- characterization tests for the parallel walk (fork-local perf work) ---
    // These pin the exact result SET, so `build_parallel` must reproduce the output a
    // serial walk would produce. cwd during `cargo test` is the crate root.
    ...
    #[test]
    fn parallel_walk_finds_known_source_file() {
        let files = walk_matches("src", "*.rs", false, false, None);
    ...
    #[test]
    fn parallel_walk_is_deduped() {
        let files = walk_matches("src", "*", false, false, None);
    ...
    #[test]
    fn parallel_walk_respects_gitignore() {
        // target/ is gitignored — nothing under it should appear.
        let files = walk_matches(".", "*", false, false, None);
        assert!(
    ...
    #[test]
    fn parallel_walk_dotfile_pattern_includes_hidden() {
        // Pattern starts with '.', so hidden entries are walked (#1101).
        let files = walk_matches(".", ".gitignore", false, false, Some(1));
        assert!(files.contains(&".gitignore".to_string()));
    }

    #[test]
    fn parallel_walk_regular_pattern_skips_hidden() {
        // Non-dot pattern: hidden top-level entries (.gitignore, .github) are skipped.
        let files = walk_matches(".", "*", false, false, Some(1));
        assert!(
    ...
    #[test]
    fn parallel_walk_type_dir_returns_dirs_not_files() {
        let dirs = walk_matches("src", "*", true, false, None);
    ...
    #[test]
    fn parallel_walk_type_file_excludes_dirs() {
        let files = walk_matches("src", "*", false, false, None);
        assert!(
    ...
    #[test]
    fn parallel_walk_iname_is_case_insensitive() {
        let ci = walk_matches(".", "cargo.toml", false, true, Some(1));
        assert!(
            ci.contains(&"Cargo.toml".to_string()),
            "case-insensitive should match Cargo.toml"
        );
        let cs = walk_matches(".", "cargo.toml", false, false, Some(1));
        assert!(
    ...
    #[test]
    fn parallel_walk_maxdepth_bounds_descent() {
        // depth 1 in src => only entries directly in src/, no nested paths.
        let shallow = walk_matches("src", "*.rs", false, false, Some(1));
        assert!(
            !shallow.is_empty(),
            "expected at least one top-level .rs in src"
        );
        assert!(
            shallow
                .iter()
        ...
        let deep = walk_matches("src", "*.rs", false, false, None);
        assert!(
    ...
    #[test]
    fn parallel_walk_no_matches_is_empty() {
        let files = walk_matches("src", "*.xyz_nonexistent", false, false, None);
        assert!(files.is_empty());
    }
}
